...
개인 임시 메모, AI 초안, 조사 원문, 실험 기록은 루트의 `docs-local/`에 둡니다.

- `docs-local/`은 `.gitignore`에 등록되어 Git에 포함되지 않습니다.
- 공통 검토가 필요한 내용은 정리한 뒤 `docs/`에 새 문서 또는 수정으로 반영합니다.
- 원본 추출물, 외부 에셋, 비공개 정보는 `docs/`에 넣지 않습니다. 단, 팀이 공동 기준으로 사용하도록 제공했고 저장소 공유 권한을 확인한 기술 레퍼런스는 `docs/technical-foundation/reference/`에 보존할 수 있습니다.
